...
class TikhPGSolver:
    def __init__(self, zexp_re, zexp_im, omg, lamT0, lampg0, df, fname, mode='real'):
        ...
        self.spline_reg = jnp.sum(jnp.diff(self.am, 2)**2)
        self.gfun_init = self.Tikh_solver(self.lamT0, self.amTam, self.brs, self.Idm, self.spline_reg)
        if mode == 'real':
            self.fsuffix = 'zre'
        else:
            self.fsuffix = 'zim'
    ...
    def Tikh_solver(self, lamt, amTam, brs, Idm, spline_reg):
        amTikh = amTam + lamt * Idm * spline_reg  # --- new Tikhonov matrix
        sol, residuals, rank, sv = jnp.linalg.lstsq(amTikh, brs, rcond=-1)
        return sol
    ...
    def pg_solver(self, lamvec, amTikh, amTam, brs, dlntau, Idm, spline_reg):
        lamT, lampg = lamvec  # These are the two regularization parameters and the regularization parameter for the spline.
        gtau = self.Tikh_solver(lamT, amTam, brs, Idm, spline_reg)  # Initial Gfun from Tikhonov solver
        pg = jaxopt.ProjectedGradient(fun=jax.jit(self.objective_fun), projection=jaxopt.projection.projection_non_negative, tol=1e-8, maxiter=self.niter * 1000, implicit_diff=True, jit=True)
    ...
    def Tikh_residual(self, lamvec, amTikh, amTam, brs, dlntau, Idm, spline_reg):   # --- returns vector of Tikhonov residuals to be minimized
        gfvec, rp, kk = self.pg_solver(lamvec, amTikh, amTam, brs, dlntau, Idm, spline_reg)   # --- (new amTikh has been calculated)
        resid = jnp.matmul(self.amTikh, gfvec) - self.brs
        return resid


    def jacoby(self, pvec, amTikh, amTam, brs, dlntau, Idm, spline_reg):
        return (jax.jacobian(self.Tikh_residual)(jnp.array(pvec), amTikh, amTam, brs, dlntau, Idm, spline_reg))
    ...
    def find_lambda(self):
        ...
        for k in range(kmax):
            ...
            gfun = self.Tikh_solver(lam1, self.amTam, self.brs, self.Idm, self.spline_reg)
    ...
    def driver(self, lsq):   # --- omega must be in descending order!
        ...
        if lsq == 'trf':
            resparm = least_squares(jax.jit(self.Tikh_residual), lamvecinit,
                                    method='lm', jac=self.jacoby, args=(self.amTikh, self.amTam, self.brs, self.dlntau, self.Idm, self.spline_reg))

        else:
            resparm = least_squares(jax.jit(self.Tikh_residual), lamvecinit, bounds=(low, high),
                                     method='trf', jac=self.jacoby, args=(self.amTikh, self.amTam, self.brs, self.dlntau, self.Idm, self.spline_reg))

        res = resparm.x

        print(f"resparm.x = {res}")

        gfun, rpoly, nit = self.pg_solver(res, self.amTikh, self.amTam, self.brs, self.dlntau, self.Idm, self.spline_reg)
