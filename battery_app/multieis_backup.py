...
class Multieis:
    ...
    def __init__(
        ...
        freq: jnp.ndarray,
        Z: jnp.ndarray,
        bounds: Sequence[Union[Tuple[float, float], Sequence[float]]],
        smf: jnp.ndarray,
        func: Callable[[float, float], float],
        immittance: str = "impedance",
        weight: Optional[Union[None, str, jnp.ndarray]] = None,
    ) -> None:
        ...
        self.indices = None
        self.n_fits = None
        self.popt_log_1 = None
        self.popt_log_2 = None
        self.popt_log_3 = None
    ...
    def convert_to_external(self,
        ...
        """
        up = (10 ** jnp.take(P, self.gather_indices))
        par_ext = (
            jnp.take(self.lb_vec, self.gather_indices)
            + up
        ) / (
            1
            + up
            / jnp.take(self.ub_vec, self.gather_indices)
        )
        return par_ext

    def convert_to_external_spline(self,
                            P: jnp.ndarray
                            ) -> jnp.ndarray:
        ...
        :returns: Parameters in normal scale
        """
        up = (10 ** P.reshape(-1, self.num_eis))
        par_ext = (
            jnp.take(self.lb_vec, self.gather_indices)
            + up
        ) / (
    ...
    def compute_total_obj(self,
        ...
        return (jnp.nansum(wrss_tot) + chi_smf)

    def compute_total_obj_spline(self,
                          P: jnp.ndarray,
                          F: jnp.ndarray,
                          Z: jnp.ndarray,
                          Zerr_Re: jnp.ndarray,
        ...
        # P_log = jnp.sum(spline_basis.reshape(1, self.num_eis, self.num_basis_funcs) * P.reshape(self.num_params, 1, self.num_basis_funcs), axis = -1)
        P_log = jnp.einsum('bij,bij->bi', spline_basis.reshape(1, self.num_eis, self.num_basis_funcs), P.reshape(self.num_params, 1, self.num_basis_funcs), optimize = True)
        up = (10 ** P_log)
        P_norm = (
            jnp.take(LB, self.gather_indices)
            + up
    ...
    def compute_perr_spline(self,
        ...
        # P_log = jnp.sum(spline_basis.reshape(1, self.num_eis, self.num_basis_funcs) * P.reshape(self.num_params, 1, self.num_basis_funcs), axis = -1)
        P_log = jnp.einsum('bij,bij->bi', spline_basis.reshape(1, self.num_eis, self.num_basis_funcs), P.reshape(self.num_params, 1, self.num_basis_funcs), optimize = True)
        # P_log = jax.vmap(jnp.dot, in_axes = (None, 0), out_axes = 0)(spline_basis, P)
        ...
        chitot = self.compute_total_obj_spline(P, F, Z, Zerr_Re, Zerr_Im, LB, UB, spline_basis)/self.dof
        print(chitot)
        hess_mat = jax.hessian(self.compute_total_obj_spline)(P, F, Z, Zerr_Re, Zerr_Im, LB, UB, spline_basis)
        print('done with hess_mat')
        try:
            # Here we check to see if the Hessian matrix is singular \
            # or ill-conditioned since this makes accurate computation of the
            # confidence intervals close to impossible.
        ...
        std_error = jnp.sqrt(jnp.diag(self.cov_mat)).reshape(self.num_params, self.num_basis_funcs)
        perr = jnp.zeros(shape=(self.num_params, self.num_basis_funcs))
        for i in range(self.num_params):
            perr = perr.at[i, :].set(std_error[i, :])
        # perr = jnp.sum(spline_basis.reshape(1, self.num_eis, self.num_basis_funcs) * perr.reshape(self.num_params, 1, self.num_basis_funcs), axis = -1) * popt
        # Here we assume that the parameters are linear with respect to the spline coefficients
        perr = jnp.einsum('bij,bij->bi', spline_basis.reshape(1, self.num_eis, self.num_basis_funcs), perr.reshape(self.num_params, 1, self.num_basis_funcs), optimize = True) * popt
    ...
    def train_step_spline(self,
        ...
        net_params = self.get_params(opt_state)
        self.loss, self.grads = jax.value_and_grad(
            self.compute_total_obj_spline, argnums=0
            )(
                net_params,
                F,
                Z,
    ...
    def fit_simultaneous_spline(self,
                         method : str = 'TNC',
                         n_iter : int = 5000,
                         df: int = 5,
                         ) -> Tuple[
        ...
        """
        self.method = method.lower()
        # The bs function generates a cubic spline basis for the given input data. In this case, 
        # the input data is the cnt array, which is a linearly spaced array ranging from 0 to 1 with a length of self.num_eis. 
        # The knots argument specifies the knots (breakpoints) for the spline, which are defined as the knot_list array in this case. 
        # The degree argument specifies the degree of the spline, which is 3 for a cubic spline. 
        # The include_intercept argument specifies whether to include an intercept column in the spline matrix, which is set to True in this case.
        # The resulting spline matrix is then subtracted by a matrix of ones to ensure that the spline matrix has a mean of 0. 
        # The resulting spline matrix has self.num_eis rows and num_knots columns, and it represents the spline basis for the input data.
        spline_basis = jnp.asarray(dmatrix("bs(cnt, df=df, degree=3, include_intercept=True) - 1",
        {"cnt": jnp.arange(0, self.num_eis), "df": df}))
        self.num_basis_funcs = spline_basis.shape[1]
        ...
        if hasattr(self, "popt") and self.popt.shape[1] == self.Z.shape[1]:
            print("\nUsing prefit")
            p_log = (
                self.encode(self.check_nan_values(self.popt[:, 0]), self.lb, self.ub)
            )
            self.par_log = jnp.broadcast_to(p_log[:,None], (self.num_params, self.num_basis_funcs)).flatten()

        else:
            print("\nUsing initial")
            p_log = (
                self.encode(self.p0, self.lb, self.ub)
            )
            self.par_log = jnp.broadcast_to(p_log[:,None], (self.num_params, self.num_basis_funcs)).flatten()
        ...
        solver = jaxopt.ScipyMinimize(
            method=self.method,
            fun=jax.jit(self.compute_total_obj_spline),
            dtype='float64',
            tol=1e-14,
            maxiter=n_iter,
            )
        ...
        popt_spline = self.sol.params
        self.popt_log_1 = spline_basis.reshape(1, self.num_eis, self.num_basis_funcs) * self.par_log.reshape(self.num_params, 1, self.num_basis_funcs)
        self.popt_log_2 = spline_basis.reshape(1, self.num_eis, self.num_basis_funcs) * popt_spline.reshape(self.num_params, 1, self.num_basis_funcs)
        popt_log = jnp.einsum('bij,bij->bi', spline_basis.reshape(1, self.num_eis, self.num_basis_funcs), popt_spline.reshape(self.num_params, 1, self.num_basis_funcs), optimize=True).flatten()
        self.popt_log_3 = popt_log.copy().reshape(self.num_params, self.num_eis)
        self.popt = self.convert_to_external_spline(popt_log)
        self.chitot = self.sol.state.fun_val/self.dof

        self.perr = jax.jit(self.compute_perr_spline)(
            popt_spline,
            self.F,
            self.Z,
            self.Zerr_Re,
    ...
    def fit_stochastic_spline(self,
                       lr: float = 1e-3,
                       num_epochs: int = 100000,
                       df: int = 4,
                       ) -> Tuple[
        ...
        for epoch in range(self.num_epochs):
            ...
            self.loss, self.opt_state = jax.jit(self.train_step_spline)(
                epoch,
                self.opt_state,
                self.F,
                self.Z,
        ...
        self.chitot = self.losses[-1]
        popt_spline = self.get_params(self.opt_state)
        popt_log = jnp.sum(spline_basis.reshape(1, self.num_eis, self.num_basis_funcs) * popt_spline.reshape(self.num_params, 1, self.num_basis_funcs), axis = -1).flatten()
        ...
        # Compute perr using the fractional covariance matrix

        self.perr = self.compute_perr_spline(
            popt_spline,
            self.F,
            self.Z,
            self.Zerr_Re,
    ...
    def fit_sequential(self,
                       indices: Union[None, Sequence[int]] = None,
                       ) -> Tuple[
                        jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray
                        ]:
        """
